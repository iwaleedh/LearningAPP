...
def process_files():
    ...
    for filename, svgs in svg_data.items():
        ...
        new_blocks_str = ""
        for i, (svg_id, caption, svg_content) in enumerate(svgs):
      ...
      data: {{
        ...
        match = re.search(r'(\n\s*\]\,\n\s*"?recall"?:)', content)
        if match:
            pre_match_idx = match.start()
            pre_content = content[:pre_match_idx]
            
            pre_content_stripped = pre_content.rstrip()
            has_comma = pre_content_stripped.endswith(',')
            ...
            replacement += new_blocks_str + match.group(1)
            
            new_content = pre_content_stripped + replacement + content[match.end():]
