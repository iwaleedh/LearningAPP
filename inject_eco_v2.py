...
with open('ecou4.json', 'r', encoding='utf-8') as f:
    tips_dict = json.load(f)

for filename, tips_array in tips_dict.items():
    filepath = os.path.join('src/data/seedNotes/economics', filename)
    ...
    for idx, tip in enumerate(tips_array):
        tip_id = f"callout-examiner-{idx+1}"
        if tip_id in content:
            continue

        title_escaped = json.dumps(tip.get('title', 'Examiner Tip'))
