...
def inject_block(filepath, target_id, new_block_str, append_comma=False):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Regex to find the block up to its closing }
    # We use a trick: match the target id, then look for the closing curly bracket that aligns with the opening one.
    # Since it's hard with simple regex to do balanced brackets, we can just match up to \n    \} (since the indentation is 4 spaces).
    
    # Let's match from { id: 'target_id' up to `    }` or `        }`.
    ...
    replacement = r"\1" + ("," if append_comma else "") + "\n" + new_block_str
    
    # if the original block had a comma AFTER it, that comma is NOT in \1. 
    # So if it was `},\n`, replacing `}` with `}, \n new_block` will make `},,\n new_block`.
    # Actually wait. If we just match `\}` and NOT the comma, and we replace with `},\n` it's better to just do `\n`.
    # Let's write a smarter regex.
    pattern2 = r"(\{\s*id:\s*['\"]" + target_id + r"['\"].*?\n\s*\}(?:,))"
    ...
    if match:
        ...
        # maybe it doesn't have a comma (last item)
        pattern3 = r"(\{\s*id:\s*['\"]" + target_id + r"['\"].*?\n\s*\})(?!\s*,)"
        replacement = r"\1,\n" + new_block_str
