...
svg_str = """
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 400" width="100%" height="auto" font-family="Inter,Arial,sans-serif">
    <!-- Background Grid -->
    <defs>
...
file_path = 'src/data/seedNotes/chemistry/note_2_6_3.js'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# We want to insert it after the list-cooling block.
# We'll search for:
#             id: "list-cooling",
#             type: "list",
#             data: {
...
pattern = re.compile(r'(id:\s*["\']list-cooling["\'][\s\S]*?terms:\s*\[\]\s*\}\s*)', re.MULTILINE)

match = pattern.search(content)

if match:
    # Insert right after match group 1 ending brace and comma
    insertion_point = match.end()
    
    # Needs a comma if not there? The existing block usually ends with:
    #         },
    # So we just append our block.
    # Wait, the match is `}\n` we need to add a comma if it's the last element, but wait, recall comes after. 
    # Let's just find `id: "list-cooling",` block and append our block. 
    # To be safer, let's substitute `terms: []\n        }` with `terms: []\n        },\n` + svg_block
    
    block_end_pattern = re.compile(r'(id:\s*["\']list-cooling["\'][\s\S]*?terms:\s*\[\]\s*)\}')
    
    new_content = block_end_pattern.sub(r'\1},\n' + svg_block, content)
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully injected cooling curve SVG into note_2_6_3.js")
else:
    print("Could not find list-cooling block in note_2_6_3.js")
